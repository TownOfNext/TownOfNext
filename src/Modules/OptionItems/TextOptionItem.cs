...
public class TextOptionItem : OptionItem
{
    ...
    public static TextOptionItem Create(
        GameModeInfo modeInfo, int idOffset, string name, bool isSingleValue = false, OptionItem parent = null
    )
    {
        var opt = new TextOptionItem(
            modeInfo.ConfigId + idOffset, name, 0, TabGroup.GameSettings, isSingleValue
        );
