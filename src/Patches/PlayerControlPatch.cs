...
class FixedUpdatePatch
{
    ...
    public static void Postfix(PlayerControl __instance)
    {
        ...
        {//実行クライアントがホストの場合のみ実行
            ...
            if (GameStates.IsInTask && ReportDeadBodyPatch.CanReport[player.PlayerId] && ReportDeadBodyPatch.WaitReport[player.PlayerId].Count > 0)
            {
                var info = ReportDeadBodyPatch.WaitReport[player.PlayerId][0];
                ReportDeadBodyPatch.WaitReport[player.PlayerId].Clear();
                Logger.Info($"{player.GetNameWithRole()}:通報可能になったため通報処理を行います", "ReportDeadbody");
                player.ReportDeadBody(info);
            }
            //踢出低等级的人
            ...
            if (GameStates.IsLobby)
            {
                if (Main.playerVersion.TryGetValue(__instance.PlayerId, out var ver))
                {
                    if (Main.ForkId != ver.forkId) // フォークIDが違う場合
                        ...
                        __instance.cosmetics.nameText.text = ver.tag == $"{Main.GitCommit}({Main.GitBranch})" ? $"<color=#fcc5f8>{__instance.name}</color>" : $"<color=#ffff00><size=1.5>{ver.tag}</size>\n{__instance?.name}</color>";
    ...
    {
        if (Lovers.LoverSuicide.GetBool() && CustomRoles.Lovers.IsExist(true) && !Main.isLoversDead)
        {
...
class CoEnterVentPatch
{
    ...
    public static bool Prefix(PlayerPhysics._CoEnterVent_d__47 __instance)
    {
        ...
        )
        {
            _ = new LateTask(() =>
            {
                if (!GameStates.IsMeeting && user.inVent) playerPhysics.RpcBootFromVent(id);
