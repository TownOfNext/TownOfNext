        ...
        public static void Postfix(GameStartManager __instance)
        {
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                if (!canStartGame)
                {
                    ...
                    warningMessage = Utils.ColorString(Color.red, string.Format(GetString("Warning.MismatchedVersion"), string.Join(" ", mismatchedPlayerNameList), $"<color={Main.ModColor}>{(Main.IsAprilFools ? "TOHE" : Main.ModName)}</color>"));
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (exitTimer != 0)
                        warningMessage = Utils.ColorString(Color.red, string.Format(GetString("Warning.AutoExitAtMismatchedVersion"), $"<color={Main.ModColor}>{(Main.IsAprilFools ? "TOHE" : Main.ModName)}</color>", Math.Round(5 - exitTimer).ToString()));
                }
            }
            if (warningMessage == "")
            {
