...
public static class GuesserHelper
{
    public static bool GuesserMsg(PlayerControl pc, string msg, out bool spam)
    {
        ...
        if (!pc.GetRoleAndAddonClasses()?.OfType<IGuesser>().Any() ?? false) return false;
    ...
    public static void ShowGuessPanel(byte playerId, MeetingHud __instance)
    {
        ...
        try
        {
            ...
            IGuesser gc = PlayerControl.LocalPlayer.GetRoleAndAddonClasses()?.OfType<IGuesser>().FirstOrDefault();
            int tabCount = 0;
