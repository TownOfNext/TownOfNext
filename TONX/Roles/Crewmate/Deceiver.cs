...
public sealed class Deceiver : RoleBase, IKiller
{
    ...
    {
        OptionSellCooldown = FloatOptionItem.Create(RoleInfo, 10, OptionName.DeceiverSkillCooldown,
                new(2.5f, 180f, 2.5f), 20f, false)
            .SetValueFormat(OptionFormat.Seconds);
        OptionSellNums = IntegerOptionItem
            .Create(RoleInfo, 11, OptionName.DeceiverSkillLimitTimes, new(1, 15, 1), 2, false)
    ...
    public bool OnCheckMurderAsKiller(MurderInfo info)
    {
        ...
        Logger.Info($"{killer.GetNameWithRole()}：将赝品售卖给 => {target.GetNameWithRole()}",
            "Deceiver.OnCheckMurderAsKille");
    ...
    private static bool OnCheckMurderPlayerOthers_Before(MurderInfo info)
    {
        ...
        foreach (var deceiver in Main.AllPlayerControls.Where(x => x.Is(CustomRoles.Deceiver)))
        {
            ...
            if (roleClass.Customers.TryGetValue(killer.PlayerId, out var x) && x)
            {
                ...
                Logger.Info($"{deceiver.GetNameWithRole()} 的客户：{killer.GetNameWithRole()} 因使用赝品走火自杀",
                    "Deceiver.OnCheckMurderPlayerOthers_Before");
                return false;
            }
        }
    ...
    public override void OnStartMeeting()
    {
        ...
        foreach (var pcId in Customers.Keys)
        {
            ...
            Logger.Info($"赝品商 {Player.GetRealName()} 的客户 {target.GetRealName()} 因不带刀将在会议结束后自杀",
                "Deceiver.OnStartMeeting");
        }
    }
    ...
    public override string GetProgressText(bool comms = false) =>
        Utils.ColorString(CanUseKillButton() ? RoleInfo.RoleColor : Color.gray, $"({SellLimit})");
}