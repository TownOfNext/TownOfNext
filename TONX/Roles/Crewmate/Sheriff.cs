...
public sealed class Sheriff : RoleBase, IKiller, ISchrodingerCatOwner
{
    ...
    private static void SetupOptionItem()
    {
        KillCooldown = FloatOptionItem
            .Create(RoleInfo, 10, GeneralOption.KillCooldown, new(2.5f, 180f, 2.5f), 15f, false)
        ...
        CanKillNeutralsMode = StringOptionItem.Create(RoleInfo, 14, OptionName.SheriffCanKillNeutralsMode, KillOption,
            0, false, CanKillNeutrals);
        SetUpNeutralOptions(30);
        SetMadCanKill = BooleanOptionItem.Create(RoleInfo, 18, OptionName.SheriffSetMadCanKill, false, false);
        MadCanKillImp =
            BooleanOptionItem.Create(RoleInfo, 19, OptionName.SheriffMadCanKillImp, true, false, SetMadCanKill);
        MadCanKillNeutral = BooleanOptionItem.Create(RoleInfo, 20, OptionName.SheriffMadCanKillNeutral, true, false,
            SetMadCanKill);
        MadCanKillCrew =
            BooleanOptionItem.Create(RoleInfo, 21, OptionName.SheriffMadCanKillCrew, true, false, SetMadCanKill);
    }
    ...
    public static void SetUpKillTargetOption(CustomRoles role, int idOffset, bool defaultValue = true,
        OptionItem parent = null)
    {
        ...
        Dictionary<string, string> replacementDic = new()
            { { "%role%", Utils.ColorString(Utils.GetRoleColor(role), roleName) } };
        KillTargetOptions[role] = BooleanOptionItem
            .Create(id, OptionName.SheriffCanKill + "%role%", defaultValue, RoleInfo.Tab, false).SetParent(parent);
        KillTargetOptions[role].ReplacementDictionary = replacementDic;
    }

    public static void SetUpSchrodingerCatKillTargetOption(SchrodingerCat.TeamType catType, int idOffset,
        bool defaultValue = true, OptionItem parent = null)
    {
        var id = RoleInfo.ConfigId + idOffset;
        parent ??= RoleInfo.RoleOption;
        // (%team%陣営)
        var inTeam = GetString("In%team%",
            new Dictionary<string, string>() { ["%team%"] = GetRoleString(catType.ToString()) });
        // シュレディンガーの猫(%team%陣営)
        var catInTeam = Utils.ColorString(SchrodingerCat.GetCatColor(catType),
            Utils.GetRoleName(CustomRoles.SchrodingerCat) + inTeam);
        Dictionary<string, string> replacementDic = new() { ["%role%"] = catInTeam };
        SchrodingerCatKillTargetOptions[catType] = BooleanOptionItem
            .Create(id, OptionName.SheriffCanKill + "%role%", defaultValue, RoleInfo.Tab, false).SetParent(parent);
    ...
    public static bool CanBeKilledBy(PlayerControl player)
    {
        ...
        if (player.GetRoleClass() is SchrodingerCat schrodingerCat)
        {
            ...
            {
                SchrodingerCat.TeamType.Mad => KillTargetOptions.TryGetValue(CustomRoles.Madmate, out var option) &&
                                               option.GetBool(),
                SchrodingerCat.TeamType.Crew => false,
                _ => CanKillNeutrals.GetValue() == 0 ||
                     (SchrodingerCatKillTargetOptions.TryGetValue(schrodingerCat.Team, out var option) &&
                      option.GetBool()),
            };
        }
        ...
        return cRole.GetCustomRoleTypes() switch
        {
            ...
            CustomRoleTypes.Neutral => CanKillNeutrals.GetBool() && (CanKillNeutralsMode.GetValue() == 0 ||
                                                                     (!KillTargetOptions.TryGetValue(cRole,
                                                                         out var option) || option.GetBool())),
            _ => CanKill,
        };
    }
}