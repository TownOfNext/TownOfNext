...
public sealed class FortuneTeller : RoleBase
{
    ...
    {
        OptionCheckNums = IntegerOptionItem
            .Create(RoleInfo, 10, OptionName.FortuneTellerSkillLimit, new(1, 15, 1), 5, false)
    ...
    public override bool CheckVoteAsVoter(PlayerControl votedFor)
    {
        ...
        if (Is(votedFor))
        {
            string notice1 = GetString("FortuneTellerCheckSelfMsg") + "\n\n" +
                             string.Format(GetString("FortuneTellerCheckLimit"), CheckLimit) +
                             GetString("SkillDoneAndYouCanVoteNormallyNow");
            Player.ShowPopUp(notice1);
            Utils.SendMessage(notice1, Player.PlayerId,
                Utils.ColorString(Utils.GetRoleColor(CustomRoles.FortuneTeller),
                    GetString("FortuneTellerCheckMsgTitle")));
            return false;
        }
        ...
        {
            msg = string.Format(GetString("FortuneTellerCheck.TaskDone"), votedFor.GetRealName(),
                GetString(votedFor.GetCustomRole().ToString()));
        }
        else
        {
        ...
        Utils.SendMessage(notice2, Player.PlayerId,
            Utils.ColorString(Utils.GetRoleColor(CustomRoles.FortuneTeller), GetString("FortuneTellerCheckMsgTitle")));
