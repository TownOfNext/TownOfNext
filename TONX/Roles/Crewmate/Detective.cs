...
public sealed class Detective : RoleBase
{
    ...
    public override void OnReportDeadBody(PlayerControl reporter, NetworkedPlayerInfo target)
    {
        ...
        if (reporter == null || !Is(reporter) || target == null || tpc == null ||
            reporter.PlayerId == target.PlayerId) return;
        {
            string msg;
    ...
    public override void NotifyOnMeetingStart(ref List<(string, byte, string)> msgToSend)
    {
        if (MsgToSend != null)
            msgToSend.Add((MsgToSend, Player.PlayerId,
                Utils.ColorString(RoleInfo.RoleColor, GetString("DetectiveNoticeTitle"))));
        MsgToSend = null;
    }
}