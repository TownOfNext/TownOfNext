...
public sealed class Bodyguard : RoleBase
{
    ...
    {
        OptionRadius = FloatOptionItem
            .Create(RoleInfo, 10, OptionName.BodyguardProtectRadius, new(0.5f, 5f, 0.5f), 1.5f, false)
        ...
        foreach (var pc in Main.AllAlivePlayerControls.Where(x => x.PlayerId != target.PlayerId))
        {
            ...
            if (pc.Is(CustomRoles.Bodyguard))
            {
                ...
                else
                {
                    ...
                    Logger.Info($"{pc.GetRealName()} 挺身而出与歹徒 {killer.GetRealName()} 同归于尽",
                        "Bodyguard.OnCheckMurderPlayerOthers_After");
                    return false;
                }
            }
        }
