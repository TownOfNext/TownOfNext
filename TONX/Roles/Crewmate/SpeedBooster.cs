...
public sealed class SpeedBooster : RoleBase
{
    ...
    {
        OptionUpSpeed = FloatOptionItem
            .Create(RoleInfo, 10, OptionName.SpeedBoosterUpSpeed, new(0.1f, 1.0f, 0.1f), 0.2f, false)
            .SetValueFormat(OptionFormat.Multiplier);
        OptionSpeedBoosterTimes = IntegerOptionItem
            .Create(RoleInfo, 11, OptionName.SpeedBoosterTimes, new(1, 99, 1), 5, false)
    ...
    public override bool OnCompleteTask(out bool cancel)
    {
        ...
        if (!MyTaskState.HasCompletedEnoughCountOfTasks(BoostTimes))
        {
            ...
            if (Main.AllPlayerSpeed[playerId] > 3) Player.Notify(Translator.GetString("SpeedBoosterSpeedLimit"));
                ...
                Player.Notify(string.Format(Translator.GetString("SpeedBoosterTaskDone"),
                    Main.AllPlayerSpeed[playerId].ToString("0.0#####")));
