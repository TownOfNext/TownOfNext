...
public static class Avenger
{
    ...
    public static void SetupCustomOption()
    {
        ...
        OptionRevengeMode = StringOptionItem
            .Create(Id + 20, "AvengerRevengeMode", revengeModes, 1, TabGroup.Addons, false)
            .SetParent(CustomRoleSpawnChances[CustomRoles.Avenger]);
        OptionRevengeNums = IntegerOptionItem
            .Create(Id + 21, "AvengerRevengeNums", new(1, 3, 1), 1, TabGroup.Addons, false)
            .SetParent(CustomRoleSpawnChances[CustomRoles.Avenger])
            .SetValueFormat(OptionFormat.Players);
        OptionRevengeOnKilled = BooleanOptionItem
            .Create(Id + 22, "AvengerRevengeOnKilled", true, TabGroup.Addons, false)
            .SetParent(CustomRoleSpawnChances[CustomRoles.Avenger]);
        OptionRevengeOnSuicide = BooleanOptionItem
            .Create(Id + 23, "AvengerRevengeOnSuicide", true, TabGroup.Addons, false)
            .SetParent(CustomRoleSpawnChances[CustomRoles.Avenger]);
    }
    ...
    public static void OnMurderPlayerOthers(MurderInfo info)
    {
        ...
        else
        {
            ...
            switch (OptionRevengeMode.GetInt())
            {
                case 1:
                    ...
                    break;
                case 2:
                    list = Main.AllAlivePlayerControls.Where(p =>
                        p.GetCustomRole().GetCustomRoleTypes() != target.GetCustomRole().GetCustomRoleTypes()).ToList();
                    break;
                case 3:
                    list = Main.AllAlivePlayerControls.Where(p =>
                        p.GetCustomRole().GetCustomRoleTypes() == target.GetCustomRole().GetCustomRoleTypes()).ToList();
        ...
        foreach (var pc in targets)
        {
            ...
            Logger.Info($"Avenger {target.GetNameWithRole()} revenged => {pc.GetNameWithRole()}",
                "Avenger.OnMurderPlayerOthers");
        }
    }
}