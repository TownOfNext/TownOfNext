...
public sealed class CursedWolf : RoleBase, IImpostor
{
    ...
    {
        OptionGuardSpellTimes = IntegerOptionItem
            .Create(RoleInfo, 10, OptionName.GuardSpellTimes, new(1, 99, 1), 3, false)
    ...
    public override bool OnCheckMurderAsTarget(MurderInfo info)
    {
        ...
        Logger.Info($"{target.GetNameWithRole()} 呪狼反杀 => {killer.GetNameWithRole()}",
            "CursedWolf.OnCheckMurderAsTarget");
        Logger.Info($"{target.GetNameWithRole()} 呪狼反杀：剩余{SpellLimit}次", "CursedWolf.OnCheckMurderAsTarget");
        return false;
    }

    public override string GetProgressText(bool comms = false) =>
        Utils.ColorString(SpellLimit >= 1 ? Color.red : Color.gray, $"({SpellLimit})");
}