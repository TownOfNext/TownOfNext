...
public sealed class Hacker : RoleBase, IImpostor
{
    ...
    public override string GetProgressText(bool comms = false) =>
        Utils.ColorString(HackLimit >= 1 ? Color.red : Color.gray, $"({HackLimit})");
    ...
    public override void OnShapeshift(PlayerControl target)
    {
        ...
        if (targetId == byte.MaxValue)
            ...
            _ = new LateTask(() => target?.NoCheckStartMeeting(Utils.GetPlayerById(targetId)?.Data), 0.15f,
                "Hacker Hacking Report");
    }
}