...
public sealed class Messenger : RoleBase, IImpostor, IKillFlashSeeable
{
    public static readonly SimpleRoleInfo RoleInfo =
        SimpleRoleInfo.Create(
            typeof(Messenger),
            player => new Messenger(player),
    ...
    private static HashSet<Messenger> instances = new(1);
    ...
    private static void HandleMurderRoomNotify(MurderInfo info)
    {
        if (canSeeMurderRoom)
        {
            foreach (var Messenger in instances)
            {
                Messenger.OnMurderPlayer(info);
            }
        }
    }
    ...
    public override void OnReportDeadBody(PlayerControl reporter, NetworkedPlayerInfo target)
    {
        ...
        var title = ColorString(Color.green, GetString("LastAdminInfo"));

        _ = new LateTask(() =>
        {
            if (GameStates.IsInGame)
            {
                SendMessage(message, Player.PlayerId, title);
            }
    ...
    public override string GetSuffix(PlayerControl seer, PlayerControl seen = null, bool isForMeeting = false)
    {
        ...
        return ColorString(Color.green, $"{GetString("MurderNotify")}: {string.Join(", ", roomNames)}");
    }
