...
public sealed class Eraser : RoleBase, IImpostor
{
    ...
    public override string GetProgressText(bool comms = false) =>
        Utils.ColorString(EraseLimit >= 1 ? Color.red : Color.gray, $"({EraseLimit})");
    ...
    public override bool CheckVoteAsVoter(PlayerControl votedFor)
    {
        ...
        {
            // 抹除中立阵营玩家
            ShowMsg(string.Format(GetString("EraserEraseNeutralNotice"), votedFor.GetRealName()) +
                    GetString("TargetInvalidAndYouShouldChooseAnotherTarget"));
        }
        else
        {
            // 正常抹除
            ...
            ShowMsg(string.Format(GetString("EraserEraseNotice"), votedFor.GetRealName()) +
        ...
        void ShowMsg(string msg)
        {
            ...
            Utils.SendMessage(msg, Player.PlayerId,
                Utils.ColorString(Utils.GetRoleColor(CustomRoles.Eraser), GetString("EraserEraseMsgTitle")));
        }
    }
