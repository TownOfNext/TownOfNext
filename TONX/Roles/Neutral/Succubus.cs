...
public sealed class Succubus : RoleBase, IKiller
{
    ...
    {
        OptionCharmCooldown = FloatOptionItem.Create(RoleInfo, 10, OptionName.SuccubusCharmCooldown,
                new(2.5f, 180f, 2.5f), 30f, false)
            .SetValueFormat(OptionFormat.Seconds);
        OptionCharmCooldownIncrese = FloatOptionItem.Create(RoleInfo, 11, OptionName.SuccubusCharmCooldownIncrese,
                new(0f, 180f, 2.5f), 10f, false)
            .SetValueFormat(OptionFormat.Seconds);
        ...
        OptionTargetKnowOtherTarget =
            BooleanOptionItem.Create(RoleInfo, 14, OptionName.SuccubusTargetKnowOtherTarget, true, false);
        OptionCharmedCountMode =
            StringOptionItem.Create(RoleInfo, 15, OptionName.CharmedCountMode, charmedCountMode, 0, false);
    }
    ...
    public float CalculateKillCooldown()
    {
        ...
        return OptionCharmCooldown.GetFloat() +
               (OptionCharmMax.GetInt() - CharmLimit) * OptionCharmCooldownIncrese.GetFloat();
    }
    ...
    public bool OnCheckMurderAsKiller(MurderInfo info)
    {
        ...
        if (CanBeCharmed(target))
        {
            ...
            killer.Notify(Utils.ColorString(Utils.GetRoleColor(CustomRoles.Succubus),
                Translator.GetString("SuccubusCharmedPlayer")));
            target.Notify(Utils.ColorString(Utils.GetRoleColor(CustomRoles.Succubus),
                Translator.GetString("CharmedBySuccubus")));
            Utils.NotifyRoles();
        ...
        killer.Notify(Utils.ColorString(Utils.GetRoleColor(CustomRoles.Succubus),
            Translator.GetString("SuccubusInvalidTarget")));
        Logger.Info($"{killer.GetNameWithRole()} : 剩余{CharmLimit}次魅惑机会", "Succubus");
        return false;
    }
    ...
    public override string GetProgressText(bool comms = false) => Utils.ColorString(
        CanUseKillButton() ? Utils.ShadeColor(RoleInfo.RoleColor, 0.25f) : Color.gray, $"({CharmLimit})");

    public static bool CanBeCharmed(PlayerControl pc) => pc != null &&
