...
public sealed class Pelican : RoleBase, IKiller, ISchrodingerCatOwner
{
    ...
    private static void SetupOptionItem()
    {
        OptionKillCooldown = FloatOptionItem
            .Create(RoleInfo, 10, OptionName.PelicanKillCooldown, new(2.5f, 180f, 2.5f), 30f, false)
    ...
    public static bool IsEaten(byte id) => Main.AllPlayerControls.Any(p =>
        p.GetRoleClass() is Pelican roleClass && (roleClass.EatenPlayers?.Contains(id) ?? false));
    ...
    public override string GetProgressText(bool comms = false) => Utils.ColorString(
        EatenPlayers.Count >= 1 ? Utils.ShadeColor(RoleInfo.RoleColor, 0.25f) : Color.gray, $"({EatenPlayers.Count})");
