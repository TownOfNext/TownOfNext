...
public static class ServerDropdownPatch
{
    ...
    public static void FillServerOptions_Postfix(ServerDropdown __instance)
    {
        List<ServerListButton> serverListButton = __instance.ButtonPool.GetComponentsInChildren<ServerListButton>()
            .Where(x => x.name != "PreviousPageButton" && x.name != "NextPageButton")
            .OrderByDescending(x => x.transform.localPosition.y).ToList();
        // 调整背景大小和位置
        __instance.background.size = new Vector2(__instance.background.size.x,
            __instance.background.size.y / serverListButton.Count * (ButtonsPerPage + 2f));
        ...
        var max = CurrentPage * ButtonsPerPage > serverListButton.Count
            ? serverListButton.Count
        ...
        foreach (ServerListButton button in serverListButton)
            if (!currentPageButton.Contains(button))
        ...
        if (PreviousPageButton == null || PreviousPageButton.gameObject == null)
            PreviousPageButton = CreateServerListButton(template, "PreviousPageButton", GetString("PreviousPage"),
        ...
        if (NextPageButton == null || NextPageButton.gameObject == null)
            NextPageButton = CreateServerListButton(template, "NextPageButton", GetString("NextPage"),
                new Vector3(0f, -1f + ButtonsPerPage * -0.5f, -1f), () =>
                {
    ...
    public static ServerListButton CreateServerListButton(ServerListButton template, string name, string text,
        Vector3 position, Action onclickaction)
    {
    ...
    {
        foreach (ServerListButton button in __instance.ButtonPool.GetComponentsInChildren<ServerListButton>())
            button.gameObject.SetActive(false);
