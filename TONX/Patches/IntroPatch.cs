...
class IntroCutscenePatch
{
    ...
    public static void ShowRole_Postfix(IntroCutscene __instance)
    {
        ...
        _ = new LateTask(() =>
        {
            ...
            if (!PlayerControl.LocalPlayer.Is(CustomRoles.Lovers) &&
                !PlayerControl.LocalPlayer.Is(CustomRoles.Neptune) && CustomRoles.Neptune.IsExist())
                __instance.RoleBlurbText.text += "\n" + Utils.ColorString(Utils.GetRoleColor(CustomRoles.Lovers),
                    GetString($"{CustomRoles.Lovers}Info"));
    ...
    public static void CoBegin_Prefix()
    {
        ...
        {
            logger.Info(
                $"{(pc.AmOwner ? "[*]" : ""),-3}{pc.PlayerId,-2}:{pc.name.PadRightV2(20)}:{pc.cosmetics.nameText.text}({Palette.ColorNames[pc.Data.DefaultOutfit.ColorId].ToString().Replace("Color", "")})");
        ...
        {
            logger.Info(
                $"{(pc.AmOwner ? "[*]" : ""),-3}{pc.PlayerId,-2}:{pc?.Data?.PlayerName?.PadRightV2(20)}:{pc.GetAllRoleName().RemoveHtmlTags()}");
        }
        ...
        foreach (var pc in Main.AllPlayerControls)
        {
            try
            {
                var text = pc.AmOwner ? "[*]" : "   ";
                text +=
                    $"{pc.PlayerId,-2}:{pc.Data?.PlayerName?.PadRightV2(20)}:{pc.GetClient()?.PlatformData?.Platform.ToString()?.Replace("Standalone", ""),-11}";
        ...
        logger.Info("------------基本设置------------");
        var tmp = GameOptionsManager.Instance.CurrentGameOptions
            .ToHudString(GameData.Instance ? GameData.Instance.PlayerCount : 10).Split("\r\n").Skip(1);
        foreach (var t in tmp) logger.Info(t);
        logger.Info("------------详细设置------------");
        foreach (var o in OptionItem.AllOptions)
            if (!o.IsHiddenOn(Options.CurrentGameMode) &&
                (o.Parent == null ? !o.GetString().Equals("0%") : o.Parent.GetBool()))
                logger.Info(
    ...
    public static bool BeginCrewmate_Prefix(IntroCutscene __instance,
        ref Il2CppSystem.Collections.Generic.List<PlayerControl> teamToDisplay)
    {
        if (PlayerControl.LocalPlayer.Is(CustomRoles.CrewPostor))
        {
            ...
            foreach (var pc in Main.AllPlayerControls.Where(x => !x.AmOwner && x.GetCustomRole().IsImpostor()))
        ...
        ref Il2CppSystem.Collections.Generic.List<PlayerControl> teamToDisplay)
    {
        ...
        switch (role)
        {
            case CustomRoles.GM:
                ...
                PlayerControl.LocalPlayer.Data.Role.IntroSound =
    ...
    public static bool BeginImpostor_Prefix(IntroCutscene __instance,
                ...
                if (Options.FixFirstKillCooldown.GetBool() && Options.CurrentGameMode != CustomGameMode.SoloKombat)
                    _ = new LateTask(() =>
                    {
                        if (GameStates.IsInTask)
                        {
                            ...
                            Main.AllPlayerControls.Where(x => (Main.AllPlayerKillCooldown[x.PlayerId] - 2f) > 0f)
                                .Do(pc => pc.SetKillCooldownV2(Main.AllPlayerKillCooldown[pc.PlayerId] - 2f));
                        }
                    }, 2f, "FixKillCooldownTask");
                _ = new LateTask(() => { CustomRoleManager.AllActiveRoles.Values.Do(x => x?.OnGameStart()); }, 0.1f,
                    "RoleClassOnGameStartTask");
            }

            _ = new LateTask(() => Main.AllPlayerControls.Do(pc => pc.RpcSetRoleDesync(RoleTypes.Shapeshifter, -3)), 2f,
                "SetImpostorForServer");
